                ...
                <form class="login-form" @submit.prevent="onSubmit">
                  <dx-form :form-data="formData" :disabled="loading" :rtl-enabled="$vuetify.rtl">
                    ...
                    <dx-item data-field='systemActivationCode' editor-type="dxTextBox"
                      :editor-options="{ stylingMode: 'filled', placeholder: 'كود تفعيل النظام' }">
                    ...
                    <dx-item>
                      <template #default>
                        <div class="link">
                          <router-link to="/Account/Activation">
                            الرجوع لصفحة التفعيل
                          </router-link>
                        </div>
                      </template>
                    </dx-item>
  ...
  export default {
    ...
    methods: {
      onSubmit: async function () {
        this.loading = true;
        const response = await this.$store.dispatch("account/generateActivationCode", this.formData);
